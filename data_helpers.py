...
EVE_DATASET_PATH = 'dataset/event_data.txt'
NEVE_DATASET_PATH = 'dataset/nevent_data.txt'
VOC_PATH = 'dataset/vocab.csv'
VOC_INV_PATH = 'dataset/vocab_inv.csv'
...
def load_data_and_labels(dataset_fraction):
    ...
    Returns the lists. 
    """
    print "\tdata_helpers: loading event examples..."
    event_examples = list(open(EVE_DATASET_PATH).readlines())
    event_examples = [s.strip() for s in event_examples]
    print "\tdata_helpers: [OK]"
    print "\tdata_helpers: loading nevent examples..."
    nevent_examples = list(open(NEVE_DATASET_PATH).readlines())
    nevent_examples = [s.strip() for s in nevent_examples]
    print "\tdata_helpers: [OK]"

    event_examples = sample_list(event_examples, dataset_fraction)
    nevent_examples = sample_list(nevent_examples, dataset_fraction)

    # Split by words
    x_text = event_examples + nevent_examples
    ...
    event_labels = [[0, 1] for _ in event_examples]
    nevent_labels = [[1, 0] for _ in nevent_examples]
    print "\tdata_helpers: [OK]"
    print "\tdata_helpers: concatenating labels..."
    y = np.concatenate([event_labels, nevent_labels], 0)
    print "\tdata_helpers: [OK]"
    return [x_text, y]
